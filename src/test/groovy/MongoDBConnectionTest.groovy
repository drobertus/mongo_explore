...
// or, to connect to a replica set, with auto-discovery of the primary, supply a seed list of members
        ...
        mapOfVals << ['dob': '2005-07-12']

        testColl.insert(new BasicDBObject(mapOfVals))

        def obj2 = new BasicDBObject('_id', 34).append('lastName', 'Thorn')
            .append('firstName', 'Marilyn').append('dob', '1968-03-22')
        ...
        while(cursor.hasNext()) {
            countReturned ++
            println countReturned + ': ' + cursor.next().toString()
        }
        cursor.close()
        def finder1 = testColl.find(new BasicDBObject('_id', 34))
